...
class ViewControllerInitial: UIViewController, UIScrollViewDelegate {
    ...
    func setupSlideScrollView(slides : [Slide]) {
        ...
        for i in 0 ..< slides.count {
            slides[i].frame = CGRect(x: 0, y: 40 * CGFloat(numberL), width: view.frame.width, height: view.frame.height)
            scrollView.addSubview(slides[i])
            numberL = numberL + slides[i].labelDesc.text.count / 56 + 1
            print(numberL)
        }
            
        let image = UIImage(named: "button-1") as UIImage?
        let button = UIButton(type: UIButton.ButtonType.custom) as UIButton
        button.frame = CGRect(x: 84, y: 40 * CGFloat(numberL), width: 244, height: 55)
        button.setImage(image, for: .normal)
        scrollView.addSubview(button)
        
        scrollView.contentSize = CGSize(width: view.frame.width , height: 40 * CGFloat(numberL + 1))
    }
}
